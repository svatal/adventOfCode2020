type IRule = { count: number; color: string }[];

export function doIt() {
  const bagRules = input
    .split(`\n`)
    .map((line) => {
      const [outerColor, innerBagsString] = line.split(" bags contain ");
      const innerBags = innerBagsString
        .split(", ")
        .filter((bag) => bag !== "no other bags.")
        .map((bag) => bag.split(" bag")[0].split(" "))
        .map(([count, ...colorWords]) => ({
          count: +count,
    ...
    .reduce(
      (map, bag) => map.set(bag.outerColor, bag.innerBags),
      new Map<string, IRule>()
    );
  const shinyGold = "shiny gold";
  function contains(rule: IRule): boolean {
    return rule.some(
      (line) => line.color === shinyGold || contains(bagRules.get(line.color)!)
    );
  }
  const first = Array.from(bagRules.values()).filter(contains).length;
  function howMany(rule: IRule): number {
    return rule
      .map((line) => line.count * (1 + howMany(bagRules.get(line.color)!)))
      .reduce((a, b) => a + b, 0);
  }
  const second = howMany(bagRules.get(shinyGold)!);
