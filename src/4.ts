function testRange(min: number, max: number) {
  return (input: string) => +input >= min && +input <= max;
}

const rules: {
  [field: string]: { regex: RegExp; isValid: (result: string) => boolean }[];
} = {
  byr: [{ regex: /^(\d{4})$/, isValid: testRange(1920, 2002) }],
  iyr: [{ regex: /^(\d{4})$/, isValid: testRange(2010, 2020) }],
  eyr: [{ regex: /^(\d{4})$/, isValid: testRange(2020, 2030) }],
  hgt: [
    { regex: /^(\d+)cm$/, isValid: testRange(150, 193) },
    { regex: /^(\d+)in$/, isValid: testRange(59, 76) },
  ],
  hcl: [{ regex: /^#[0-9a-f]{6}$/, isValid: () => true }],
  ecl: [{ regex: /^(amb|blu|brn|gry|grn|hzl|oth)$/, isValid: () => true }],
  pid: [{ regex: /^\d{9}$/, isValid: () => true }],
};

function validate(field: string, value: string): boolean {
  return rules[field]
    .map((r) => ({ re: r.regex.exec(value), isValid: r.isValid }))
    .some(({ re, isValid }) => re && isValid(re[1]));
}
...
export function doIt() {
  ...
  const requiredFieldNames = ["byr", "iyr", "eyr", "hgt", "hcl", "ecl", "pid"];
  const first = parsed.filter((fields) =>
    requiredFieldNames.every((field) => fields.has(field))
  ).length;
  const second = parsed.filter((fields) =>
    requiredFieldNames.every(
      (field) => fields.has(field) && validate(field, fields.get(field)!)
    )
  ).length;
