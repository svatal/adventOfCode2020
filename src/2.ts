export function doIt() {
  const parsed = input.split(`\n`).map((line) => {
    const [minmax, letterWithSemicolon, password] = line.split(" ");
    const [min, max] = minmax.split("-").map((i) => parseInt(i, 10));
    const letter = letterWithSemicolon[0];
    return { min, max, letter, password };
  });
  const first = parsed
    .map((line) => ({
      ...line,
      count: line.password.split("").filter((l) => l === line.letter).length,
    }))
    .filter(({ count, min, max }) => min <= count && count <= max).length;
...
export function doItFirst() {
  const parsed = input.split(`\n`).map((line) => {
    const x = line.split(" ");
    const minmax = x[0].split("-").map((i) => parseInt(i, 10));
    return {
      min: minmax[0],
      max: minmax[1],
      letter: x[1][0],
  ...
  const result = parsed.filter((line) => {
    let pointer = 0;
    console.log(line, line.pass.length);
    for (let i = 1; i <= line.max + 1; i++) {
      const letterIdx = line.pass.indexOf(line.letter, pointer);
      console.log(i, pointer, letterIdx);
      if (letterIdx < 0) {
        if (i < line.min + 1) {
          console.log("not enough");
          return false;
        }
        console.log(i <= line.max + 1);
        return i <= line.max + 1;
      } else {
        if (i === line.max + 1) {
