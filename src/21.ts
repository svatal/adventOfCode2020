...
export function doIt() {
  ...
  const candidatesM = parsed.reduce((map, f) => {
    f.allergens.forEach((a) =>
      map.set(
        a,
        map.get(a)
          ? map.get(a)!.filter((i) => f.ingredients.indexOf(i) >= 0)
          : f.ingredients
      )
    );
    return map;
  ...
  const first = parsed
    ...
    .reduce((a, b) => a + b);
  const second = Array.from(allergens.entries())
...
const input = `rgt lmpxcr ssgkt klfp dfpqn scqc vzzxl hphcb pbmhl xrlmvz fvd cpj kxl gpplt jkrsn bhlbm dkxr vrzkz drpxzr bxhsq cvz nvhnsg jtzsqznl fjbjj slglnd qdbq gnrb zpbbvvm bqb mbdksj nlxh gtzp jrlvkz ctmzsr znjp zvmnv lhsgbq pdxhgx jrcczb vrpzss qdbj lsc jhrgz hqlm qlt kh grjf brhfcbt zjsh pkmbk rncjx nmszz fzpds qglps zjsz qfnq ftfx bbkth rkzqs vgp (contains nuts, shellfish)
