export function doIt() {
  ...
  const first = lines.map((l) => solve1(l).result).reduce((a, b) => a + b);
  const second = lines.map((l) => solve2(l).result).reduce((a, b) => a + b);
...
function solve2(
  s: string,
  idx: number = 0,
  stopOnMultiply: boolean = false
): { result: number; idx: number } {
  let current = 0;
  let op: Operation = undefined;
  function applyOp(number: number) {
    switch (op) {
      case undefined:
        current = number;
        break;
  ...
  while (idx < s.length) {
    switch (s[idx]) {
      case "(": {
        const result = solve2(s, idx + 1);
        idx = result.idx;
        applyOp(result.result);
        break;
      }
      case ")":
        return { result: current, idx: idx + (stopOnMultiply ? 0 : 1) };
      case " ":
        idx++;
        break;
      case "+":
      ...
      case "*":
        if (stopOnMultiply) {
          return { result: current, idx };
        }
        op = "*";
        const result = solve2(s, idx + 1, true);
        idx = result.idx;
        applyOp(result.result);
        break;
      default: {
        const number = /^(\d+)/.exec(s.substr(idx))![1];
        idx += number.length;
        applyOp(+number);
      }
    }
...
function solve1(s: string, idx: number = 0): { result: number; idx: number } {
  let current = 0;
  let op: Operation = undefined;
  function applyOp(number: number) {
    switch (op) {
  ...
  while (idx < s.length) {
    switch (s[idx]) {
      case "(": {
        const result = solve1(s, idx + 1);
        idx = result.idx;
        applyOp(result.result);
        break;
      }
      case ")":
        return { result: current, idx: idx + 1 };
      case " ":
        idx++;
        break;
      case "+":
      case "*":
        op = s[idx] as Operation;
        idx++;
        break;
      default: {
        const number = /^(\d+)/.exec(s.substr(idx))![1];
